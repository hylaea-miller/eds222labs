...
restoration <- read_csv(here::here("week4","eelgrass.csv"),
...
H0: No difference in the portion between both restoration method

HA: The popsicle stick is better than garden staple
...
proportion <- restoration %>% 
group_by(treatment) %>% summarise(prop_success = sum(success)/n())

obs_test_stat <- proportion$prop_success[2] - proportion$prop_success[1]
